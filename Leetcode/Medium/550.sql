...
with cte as(
  select *,
  if (datediff(event_date, min(event_date) over(partition by player_id))=1, 1,0) as 'flag'
  from Activity
)

select round( sum(flag) / (select count(distinct player_id) from Activity) ,2) as 'fraction'
from cte;