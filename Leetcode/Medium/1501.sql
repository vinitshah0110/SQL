...
with cte as(
  ...
  avg(duration) over(partition by Country.name) as 'cntry_avg',
  avg(duration) over() as 'global_avg'
  from Person join Country
    on left(phone_number,3) = country_code
  join (select caller_id,duration from Calls union all select callee_id,duration from Calls) temp
    on id = temp.caller_id
)

select distinct name from cte
where cntry_avg > global_avg;